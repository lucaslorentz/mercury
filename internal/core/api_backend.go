...
// Public API
type apiBackendPublicHandler struct {
	manager *Manager
}

// Public API returns the filtered json of the workers
func (h apiBackendPublicHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
...
type apiBackendAdminHandler struct {
	manager *Manager
}

// Private API returns the unfiltered json, or executes commands
func (h apiBackendAdminHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case "GET":
