...
func (c *Cache) GetRecursive(msg *dnssrv.Msg, level int, dnsDomain string, dnsQuery uint16, dnsHost string, client net.IP, honorTTL bool) error {

	records := []dnssrv.RR{}
	switch dnsQuery {
	case dnssrv.TypeA, dnssrv.TypeAAAA:
		// www.example.com.	0	IN	A	1.2.3.4
		rs, err := c.Get(dnsDomain, "A", dnsHost, client, honorTTL)
		rs6, err := c.Get(dnsDomain, "AAAA", dnsHost, client, honorTTL)
		rs = append(rs, rs6...)
		...
		if len(records) > 0 {
			...
			c.statsAddRequestCount(rs[0].UUID())
			break
		}
		fallthrough // incase we did not find a typeA/AAAA, also check for typeCNAME
	case dnssrv.TypeCNAME:
		rs, err := c.Get(dnsDomain, dnssrv.TypeToString[dnssrv.TypeCNAME], dnsHost, client, honorTTL)
		if err == ErrNotFound {
			return err
		}
		records, err = dnsRecordToRR(rs)
