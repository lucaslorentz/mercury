...
func (c *Cache) GetRecursiveForward(level int, dnsDomain string, dnsQuery uint16, dnsHost string) (rs []Record, err error) {
	...
	/*
		// Check if we have record in cache before searching the internet
		fmt.Printf("level:%d get domain\n", level)
		rs, err = c.Get(dnsDomain, dnssrv.TypeToString[dnsQuery], dnsHost, client, honorTTL)
		if err == ErrNotFound {
			fmt.Printf("level:%d get domain not found\n", level)
			// Get Closest available NS
			fmt.Printf("level:%d get ns\n", level)
			rs, err = c.Get(dnsDomain, "NS", dnsHost, client, honorTTL)
			if err == ErrNotFound {
				fmt.Printf("level:%d get ns not found\n", level)
				// get 1 level up
				var domain string
				if dnsHost == "" {
					_, domain = splitDomain(dnsDomain)
				...
				fmt.Printf("level:%d get ns recursive -> %d\n", level, level+1)
				rs, err = c.GetRecursiveForward(level+1, domain, dnssrv.TypeNS, "")
				fmt.Printf("level:%d got ns recursive -> %d\n", level, level+1)
				if err == ErrNotFound {
					fmt.Printf("level:%d error2 for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
					return []Record{}, ErrNotFound
				}
				fmt.Printf("level:%d got ns\n", level)
			}
			// We now have NS records
			fmt.Printf("level:%d we have NS, get A for host:%s domain:%s query:%d ns:%v\n", level, dnsHost, dnsDomain, dnsQuery, rs)

			// get A records for DNS:
			var nsA []string
			var nsAAAA []string
			for _, record := range rs {
				if record.Type == "A" { // TODO: ipv6 support for doing remote queries with an ipv6 addr
			...
			//fmt.Printf("Found NS's: %+v", rs)
			fmt.Printf("level:%d call resolve on found NS for host:%s domain:%s query:%d ns:%v\n", level, dnsHost, dnsDomain, dnsQuery, nsA)
			rs, err = Resolve(nsA, dnsHost, dnsDomain, dnsQuery)
			if err != nil {
				fmt.Printf("level:%d error1 for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
				return []Record{}, ErrNotFound
			}
		}
		fmt.Printf("level:%d received DNS records for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
		// We have a record from cache, or found aone
		// we have a NS record to query (TODO: add to cache)
		// get A record of NS servear

		// If we have more then max servers, randomize them and pick the top max


		// get supporting NS

		//fmt.Printf("Level:%d Pre-Add: %v\n", level, rs)
		fmt.Printf("level:%d got something for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
		switch dnsQuery {
		case dnssrv.TypeA, dnssrv.TypeAAAA:
			fmt.Printf("Level:%d Got A/AAAA", level)
			for _, record := range rs {
				if record.Type == "CNAME" {
					fmt.Printf("Got CNAME, try to get its name... %+v", record)
					host, domain := splitDomain(record.Target)
					rsA, err := c.GetRecursiveForward(level+1, domain, dnssrv.TypeA, host)
					fmt.Printf("Got Recursive CNAME: %v", rsA)
					if err != nil {
						fmt.Printf("level:%d error4 for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
						return rs, err
					}
		...
		case dnssrv.TypeNS:
			fmt.Printf("Level:%d === Got TypeNS: %+v\n", level, rs)
			// Add matching A records
			for _, nss := range rs {
				if nss.Type != "NS" {
					continue
				}
				...
				fmt.Printf("Level:%d Finding A records\n", level)
				// final attempt to get missing NS records from cache
				//rsA, err := c.GetRecursiveForward(20, domain, dnssrv.TypeA, host)
				rsA, err := c.Get(domain, "A", host, client, honorTTL)
				fmt.Printf("Level:%d rsA:%+v\n", level, rsA)
				if err == nil {
					rs = append(rs, rsA...)
				}
				fmt.Printf("Level:%d Found A records\n", level)
				//rs = append(rs, rsA...)
			}
		...
		// Find next NS
		fmt.Printf("level:%d result for host:%s domain:%s query:%d\n", level, dnsHost, dnsDomain, dnsQuery)
	*/
	//return rs, nil
}
